...
import { QuickInteractionService, QuickGameSession, TriviaQuestion } from './quick-interaction.service';
...
export class QuickInteractionPage implements OnInit, OnDestroy {
  ...
  private async endGame() {
    ...
    if (!this.rewardSubmitted && (this.player1Score > 0 || this.player2Score > 0)) {
      this.rewardSubmitted = true;
      const result = await this.quickInteractionSvc.endGameSession(this.gameSession?.id || `session-${Date.now()}`);

      if (result.error) {
        await this.showToast('La partida terminó, pero no se pudieron guardar los puntos', 'warning');
      } else {
