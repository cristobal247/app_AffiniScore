...
set "SUPABASE_API_KEY=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"
...
powershell -Command " $path = '%ROOT%producto\app_AffiniScore\frontend\src\environments\environment.ts'; $text = Get-Content -Path $path -Raw; $text = [regex]::Replace($text, 'supabaseUrl:\s*''[^'']*''', 'supabaseUrl: ''' + '%SUPABASE_URL%' + ''''); $text = [regex]::Replace($text, 'supabaseKey:\s*''[^'']*''', 'supabaseKey: ''' + '%SUPABASE_KEY%' + ''''); $text = [regex]::Replace($text, 'apiKeyGroq:\s*''[^'']*''', 'apiKeyGroq: ''' + '%SUPABASE_API_KEY%' + ''''); if ($text -notmatch 'supabaseKey:') { $text = $text -replace '(production:\s*false,?)', 'production: false,`n  supabaseUrl: ''%SUPABASE_URL%'',`n  supabaseKey: ''%SUPABASE_KEY%'',`n  apiKeyGroq: ''%SUPABASE_API_KEY%'',`n  devAuth: false,' } ; if ($text -notmatch 'devAuth:') { $text = $text -replace '(apiKeyGroq:\s*''[^'']*''|supabaseKey:\s*''[^'']*'')', '$&`n  devAuth: false' } ; Set-Content -Path $path -Value $text -Encoding UTF8"
